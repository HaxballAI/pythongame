...
def distancefromsideline (ball_y):
    return math.log10(min([ball_y - (pitchcornery + ballradius), pitchcornery + pitchheight - ballradius - ball_y]))

def offenseangle (blue_position, ball_position):
    x_diff = ball_position[0] - blue_position[0]
    y_diff = ball_position[1] - blue_position[1]
    theta_1 = math.atan2(y_diff, x_diff)
    x_diff_goal = pitchcornerx - blue_position[0]
    y_diff_goal = goalcornery + 0.5*goalsize - blue_position[1]
    theta_2 = math.atan2(y_diff_goal, x_diff_goal)
    if (abs(theta_1 - theta_2) < math.pi):
        return 1 + math.cos(abs(theta_1 - theta_2));
    else:
        larger_theta = max([theta_1, theta_2])
        smaller_theta = min([theta_1, theta_2])
        return 1 + math.cos((math.pi - larger_theta) + (smaller_theta + math.pi));

def eval_v3 (red_position, blue_position, ball_position, ball_velocity):
    possession_val = possession(red_position, blue_position, ball_position)
    ...
    possession_weight_defense = 0.02
    possession_weight_offense = 0.02
    goaldistance_weight = 2
    negative_velocity_weight = 0.0
    no_sideline_weight = 0.12
    offenseangleweight = 0.5
    if (redgoaldist < offensedefensecutoff):
        if (possession_val > 0.4):
            return offenseangle(blue_position, ball_position)**(offenseangleweight) * (((offensedefensecutoff**2) * redballdist/(redgoaldist**2) * (possession_val+1)**possession_weight_offense) + goingintobluegoal(ball_position, ball_velocity, blue_position) + 1000 * goingintoredgoal(ball_position, ball_velocity, red_position) - negative_velocity_weight*ball_velocity[0] + (no_sideline_weight*2) * distancefromsideline(ball_position[1]));
        return ((offensedefensecutoff**2) * redballdist/(redgoaldist**2) * (possession_val+1)**possession_weight_offense) + goingintobluegoal(ball_position, ball_velocity, blue_position) + 1000 * goingintoredgoal(ball_position, ball_velocity, red_position) - negative_velocity_weight*ball_velocity[0] + (no_sideline_weight*2) * distancefromsideline(ball_position[1]);
    elif (bluegoaldist < offensedefensecutoff):
        if (ball_position[1]>=goalcornery + ballradius and ball_position[1] <= goalcornery + goalsize - ballradius and ball_position[0] > blue_position[0]):
            return (-offensedefensecutoff/(bluegoaldist**2) * (possession_val+1)**(-0.5*possession_weight_defense)) + 1000*goingintobluegoal(ball_position, ball_velocity, blue_position) + 200*goingintoredgoal(ball_position, ball_velocity, red_position) - 1000 *(ball_position[0]-blue_position[0]) - negative_velocity_weight*ball_velocity[0] + 0.25*no_sideline_weight*distancefromsideline(ball_position[1]);
        else:
            return -((offensedefensecutoff**2) * blueballdist/(bluegoaldist**2) * (possession_val+1)**(-possession_weight_defense)) + 100 * goingintobluegoal(ball_position, ball_velocity, blue_position) + goingintoredgoal(ball_position, ball_velocity, red_position) - negative_velocity_weight*ball_velocity[0] + 0.25*no_sideline_weight*distancefromsideline(ball_position[1]);
    else:
        return 2*goingintobluegoal(ball_position, ball_velocity, blue_position) + 10*goingintoredgoal(ball_position, ball_velocity, red_position) + 100 * (possession_val+1)**(possession_weight_offense)/(redgoaldist**goaldistance_weight) - negative_velocity_weight*ball_velocity[0] + no_sideline_weight * distancefromsideline(ball_position[1]);    
...
def ballposveladjustment (ball_position, ball_velocity):
    if (ball_position[1] < pitchcornery + ballradius):
        ball_position[1] = pitchcornery + ballradius + (pitchcornery + ballradius - ball_position[1])
        ball_velocity[1] = -1.0 * ball_velocity[1]
    elif (ball_position[1] > pitchcornery + pitchheight - ballradius):
        ball_position[1] = pitchcornery + pitchheight - ballradius - (ball_position[1] - (pitchcornery + pitchheight - ballradius))
        ball_velocity[1] = -1.0 * ball_velocity[1]
    """if (ball_position[0] < pitchcornerx + ballradius):
        ball_position[0] = pitchcornerx + ballradius + (pitchcornerx + ballradius - ball_position[0])
        ball_velocity[0] = -1.0 * ball_velocity[0]
    elif (ball_position[0] > windowwidth - pitchcornerx - ballradius):
        ball_position[0] = windowwidth - pitchcornerx - ballradius - (ball_position[0] - (windowwidth - pitchcornerx - ballradius))
        ball_velocity[0] = -1.0 * ball_velocity[0]"""
    return [ball_position, ball_velocity]

def goingintobluegoal (ball_position, ball_velocity, blue_position):
        starting_pt = ball_position
        ending_pt = ball_position + 100 * ball_velocity
        velocity_slope = ball_velocity[1]/ball_velocity[0]
        ball_pos_blue_yintercept = starting_pt[1] + velocity_slope * (blue_position[0]-ball_position[0])
        if (abs(ball_pos_blue_yintercept - blue_position[1]) < 15):
            goal_yintercept = starting_pt[1] + velocity_slope * (windowwidth - pitchcornerx -  ball_position[0])
            if (ending_pt[0] >= windowwidth - pitchcornerx):
                if (goal_yintercept >= goalcornery + goalpostradius + ballradius and goal_yintercept <= goalcornery + goalsize - goalpostradius - ballradius):
                    return -0.25 -0.15 * math.log10(ending_pt[0]-(windowwidth-pitchcornerx));
                #elif (abs(goal_yintercept - (goalcornery + goalpostradius + ballradius))< 2*goalpostradius or abs(goal_yintercept - (goalcornery + goalsize - goalpostradius - ballradius)) < 2*goalpostradius):
                    #return -0.125;
            return 0;
        else:
            goal_yintercept = starting_pt[1] + velocity_slope * (windowwidth - pitchcornerx -  ball_position[0])
            if (ending_pt[0] >= windowwidth - pitchcornerx):
                if (goal_yintercept >= goalcornery + goalpostradius + ballradius and goal_yintercept <= goalcornery + goalsize - goalpostradius - ballradius):
                    return -1.0 - math.log10(ending_pt[0]-(windowwidth-pitchcornerx));
                elif (abs(goal_yintercept - (goalcornery + goalpostradius + ballradius))< 2*goalpostradius or abs(goal_yintercept - (goalcornery + goalsize - goalpostradius - ballradius)) < 2*goalpostradius):
                    return -0.5;
            return 0;

def goingintoredgoal (ball_position, ball_velocity, red_position):
        starting_pt = ball_position
        ending_pt = ball_position + 100 * ball_velocity
        velocity_slope = ball_velocity[1]/ball_velocity[0]
        ball_pos_red_yintercept = starting_pt[1] + velocity_slope * (red_position[0]-ball_position[0])
        if (abs(ball_pos_red_yintercept - red_position[1]) < 15):
            goal_yintercept = starting_pt[1] + velocity_slope * (windowwidth - pitchcornerx -  ball_position[0])
            if (ending_pt[0] <= pitchcornerx):
                if (goal_yintercept >= goalcornery + goalpostradius + ballradius and goal_yintercept <= goalcornery + goalsize - goalpostradius - ballradius):
                    return 0.25 + 0.15*math.log10(pitchcornerx - ending_pt[0]);
                elif (abs(goal_yintercept - (goalcornery + goalpostradius + ballradius))< 2*goalpostradius or abs(goal_yintercept - (goalcornery + goalsize - goalpostradius - ballradius)) < 2*goalpostradius):
                    return 0.02;
            return 0;
        else:
            goal_yintercept = starting_pt[1] + velocity_slope * (pitchcornerx -  ball_position[0])
            if (ending_pt[0] <= pitchcornerx):
                if (goal_yintercept >= goalcornery + goalpostradius + ballradius and goal_yintercept <= goalcornery + goalsize - goalpostradius - ballradius):
                    return 1.0 + math.log10(pitchcornerx - ending_pt[0]); 
                elif (abs(goal_yintercept - (goalcornery + goalpostradius + ballradius))< 2*goalpostradius or abs(goal_yintercept - (goalcornery + goalsize - goalpostradius - ballradius)) < 2*goalpostradius):
                    return 0.1;        
            return 0;
...
def update_position (redplayer, blueplayer, ball, red_move, blue_move):
    ...
    [ballpositionfinal, ballvelocityfinal] = ballposveladjustment(ballpositionfinal, ballvelocityfinal)
...
def move ():
    ...
    for bot_move in range(9):
        for human_move in range(9):
            ...
            all_eval[bot_move][human_move] = eval_v3(updated_positions[0], updated_positions[2], updated_positions[4], updated_positions[5])
    min_row_list = []
    for row in range(9):
...
def dribble(player, colour):
    ...
    if getbelowit == True:
        ...
        if colour == 'red':
            return b.pos -np.array([player.radius, 0])*4/6
        else:
            return b.pos +np.array([player.radius, 0])*4/6
    else:
