...
network = [np.random.randn(100,6) / np.sqrt(6) , np.random.randn(100,100) / 10, np.random.randn(18,100) / 10]
games = []
...
def forward(x):
    a1 = np.tanh(np.dot(network[0], x))
    a2 = np.tanh(np.dot(network[1], a1))
    a3 = logistic.cdf(2*np.dot(network[2], a2))
    return[a1, a2, a3]
...
def nnupdate(network):
    ...
    for i in range(len(games)*50):
        ...
        bluedelta2[bluechoice] = (blueresult/blueguess-(1-blueresult)/(1-blueguess))*blueguess*(1-blueguess)*2
        reddelta2[redchoice] = (redresult/redguess-(1-redresult)/(1-redguess))*redguess*(1-redguess)*2
        w2change = np.outer(bluedelta2, frame[0][1]) + np.outer(reddelta2, frame[1][1])
        bluedelta1 = np.multiply(np.dot(np.transpose(network[2]), bluedelta2), 1-(frame[0][1])**2)
        reddelta1 = np.multiply(np.dot(np.transpose(network[2]), reddelta2), 1-(frame[1][1])**2)
        w1change = np.outer(bluedelta1, frame[0][0]) + np.outer(reddelta1, frame[1][0])
        bluedelta0 = np.multiply(np.dot(np.transpose(network[1]), bluedelta1), 1-(frame[0][0])**2)
        reddelta0 = np.multiply(np.dot(np.transpose(network[1]), reddelta1), 1-(frame[1][0])**2)
        w0change = np.outer(bluedelta0, blueinput) + np.outer(reddelta0, redinput)
        print(blueresult*np.log(blueguess)+(1-blueresult)*np.log(1-blueguess) + redresult*np.log(redguess)+(1-redresult)*np.log(1-redguess))
        network = [network[0] + w0change, network[1] + w1change, network[2] + w2change]
...
while run:
    ...
    rs0 = np.dot(network[0], redinput)
    ra0 = np.tanh(rs0)
    rs1 = np.dot(network[1], ra0)
    ra1 = np.tanh(rs1)
    rs2 = np.dot(network[2], ra1)
    ra2 = logistic.cdf(2*rs2)
    redprobabilities = policy(rs2)
    redchoice = np.random.choice(range(18), 1,False, redprobabilities)
    #rederror = redwinprediction * np.log(gamedata[-1][-2]) + (1 - redwinprediction) * np.log(1 - oldredwinpred)
    #derrordoldredwinpred = (redwinprediction - oldredwinpred) / (oldredwinpred * (1 - oldredwinpred))


    bs0 = np.dot(network[0], blueinput)
    ba0 = np.tanh(bs0)
    bs1 = np.dot(network[1], ba0)
    ba1 = np.tanh(bs1)
    bs2 = np.dot(network[2], ba1)
    ba2 = logistic.cdf(2*bs2)
    blueprobabilities = policy(bs2)
    bluechoice = np.random.choice(range(18), 1,False, blueprobabilities)


    gamestate = [reds[0].pos[0], reds[0].pos[1], blues[0].pos[0], blues[0].pos[1], b.pos[0], b.pos[1]]
    gamedata.append([[ba0, ba1, ba2, bluechoice], [ra0, ra1, ra2, redchoice], gamestate])
    ...
    elif keys[pygame.K_s] or 15 <= redchoice[0] <= 16:
        reds[0].acc = np.array([0.0, 1.0])
    ...
    elif keys[pygame.K_DOWN] or 15 <= bluechoice[0] <= 16:
        blues[0].acc = np.array([0.0, 1.0])
    ...
    if b.pos[0] <= pitchcornerx:
        ...
        print(math.exp(np.average(errors)/2))
        frames = 0
        errors = []
        gamedata = []
        resetmap()
        ...
        redtoball = np.linalg.norm([b.pos[0] - reds[0].pos[0], b.pos[1] -blues[0].pos[1]])
        blueeval = ((.5/6.63)*np.log(1 + balltoredgoal) - (.5/6.63)*np.log(1 + balltobluegoal) + (.5/3.63)*(np.log(1 + bluetoball)-3.22) - (.5/3.63)*(np.log(1 + redtoball)-3.22))*.9 +.05
        games.append([gamedata, blueeval])
        randomvalues = np.random.rand(len(games) * 50)
        data = []
        errors = []
        #nnupdate
